...
export default function Home() {
  ...
  const { mutate: submitResponse, isPending } = useCreateResponse();
  const { toast } = useToast();

  const handleRevealClick = () => {
    setShowReveal(true);
    // Slight delay to allow the state update and layout shift, then smooth scroll down
    setTimeout(() => {
      window.scrollTo({
